...
/*
 * HID driver for Gigabyte Aero Keyboards
 * Copyright (c) 2019 Hemanth Bollamreddi
*/

#include <linux/hid.h>
#include <linux/module.h>
#include "aerokbd_driver.h"

MODULE_AUTHOR("Hemanth Bollamreddi <blmhemu@gmail.com>");
MODULE_DESCRIPTION("HID Keyboard driver for Gigabyte Aero Keyboards.");
...
static int aero_kbd_raw_event(struct hid_device *hdev, struct hid_report *report, u8 *rd, int size)
{
	if (report->id == 4 && size == 4)
	{
		u32 hidraw = make_u32(rd[0], rd[1], rd[2], rd[3]);
		// printk("Aero kbd raw event. hidraw code : %x", hidraw);
		switch (hidraw)
		{
		case HIDRAW_FN_F3:
...
static int aero_kbd_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	// printk("Aero kbd driver loaded.");
	int ret;
...
MODULE_DEVICE_TABLE(hid, aero_kbd_devices);

static struct hid_driver aero_kbd_driver = {
	.name = "aerokbd",
	.id_table = aero_kbd_devices,
	.probe = aero_kbd_probe,
	.raw_event = aero_kbd_raw_event};
module_hid_driver(aero_kbd_driver);
